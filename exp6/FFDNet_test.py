...
#         noisy_img = add_gaussian_noise(clean_img, sigma_val)
...
def test(model,criterion,test_loader):
    ...
    with torch.no_grad():
        for batch_idx,(data, img_name) in enumerate(test_loader):
            clean_img = data.to(device)
            noisy_img = add_gaussian_noise(clean_img,gaussian_sigma)
            noisy_img_save = noisy_img

            sigma_tensor = torch.full((clean_img.size(0),1,1,1),gaussian_sigma/255.0).to(device)
            denoised_img = model((noisy_img,sigma_tensor))
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load(os.path.join(model_path, "ffdnet.pth"), map_location=device))
    model.eval()
