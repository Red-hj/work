...
for name in os.listdir(date):
    ...
    #添加噪声
    noisy_img = noise_add(img,gaussian_sigma,salt_pepper_amount)

    #保存含噪图像
    noisy_name = f"{base}_noisy.png"
    noisy_path = os.path.join(save,noisy_name)
    cv2.imwrite(noisy_path,(noisy_img *255).astype(np.uint8))
    #ISTA
    denoised,convergence = ista(noisy_img)
    denoised = np.clip(denoised,0,1)

    #保存去噪图像
    save_name = f"{base}_denoised.png"
    save_path = os.path.join(result,save_name)
    cv2.imwrite(save_path,(denoised *255).astype(np.uint8))
